...
//add record
    ...
    if ($_FILES['user__Picture']['name'] != '') {
        ...
        $extraSql.=" ,user__Picture='{$user__Picture}' ";
    }
...
" . $extraSql;
    ...
    if (suErrorNo() > 0) {
        ...
        // picture
        if ($_FILES['user__Picture']['name'] != '') {
            @unlink(ADMIN_UPLOAD_PATH . $user__Picture);
            @unlink(ADMIN_UPLOAD_PATH . $_POST['previous_user__Picture']);
            suResize($defaultWidth, $defaultHeight, $_FILES['user__Picture']['tmp_name'], ADMIN_UPLOAD_PATH . $user__Picture);
        }
...
" . $extraSql . " WHERE user__ID='" . $_POST['user__ID'] . "'";
    ...
    if (suErrorNo() > 0) {
        ...
        suPrintJs("
            parent.window.location.href='" . ADMIN_URL . "users.php';
        ");
    }
}
